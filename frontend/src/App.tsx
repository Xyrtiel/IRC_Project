...
function App() {
  ...
  useEffect(() => {
    socket.on('connect', () => {
      ...
      socket.emit('listChannels', '', (response: ServerResponse) => {
        if (response.success) {
          setChannels(response.message.split(', '));
        }
      });
    });
        ...
        default:
          if (currentChannel) {
            socket.emit('sendMessage', 
              { channel: currentChannel, message: inputMessage }, 
              (response: ServerResponse) => {
                if (!response?.success) {
